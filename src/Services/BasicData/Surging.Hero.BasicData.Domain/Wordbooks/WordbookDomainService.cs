...
namespace Surging.Hero.BasicData.Domain.Wordbooks
{
    public class WordbookDomainService : ManagerBase, IWordbookDomainService
    {
        ...
        public async Task<bool> CheckWordbookItem(CheckWordbookInput input)
        {
            ...
            if (wordbook == null) throw new BusinessException($"系统中不存在Code为{input.WordbookCode}的字典项");
            var wordbookItems = await _wordbookItemRepository.GetAllAsync(p => p.WordbookId == wordbook.Id);
            if (wordbookItems.Any(p => p.Key == input.WordbookItemKey)) return true;
            return false;
        }
        ...
        public async Task CreateWordbook(CreateWordbookInput input)
        {
            ...
            if (wordbook != null) throw new BusinessException($"系统中已经存在code为{input.Code}的字典类型");
        ...
        public async Task CreateWordbookItem(CreateWordbookItemInput input)
        {
            ...
            if (wordbook == null) throw new BusinessException($"系统中不存在Id为{input.WordbookId}的字典类型");
            var wordbookItem =
                await _wordbookItemRepository.SingleOrDefaultAsync(p =>
                    p.Key == input.Key && p.WordbookId == input.WordbookId);
            if (wordbookItem != null) throw new BusinessException($"{wordbook.Name}已经存在key为{input.Key}的字典项");
        ...
        public async Task DeleteWordbook(long id)
        {
            ...
            if (wordbook == null) throw new BusinessException($"系统中不存在Id为{id}的字典类型");

            if (wordbook.IsSysPreset) throw new BusinessException("不允许删除系统预设的字典类型");
        ...
        public async Task DeleteWordbookItem(long id)
        {
            ...
            if (wordbookItem == null) throw new BusinessException($"系统中不存在Id为{id}的字典项");
        ...
        public async Task<GetWordbookItemOutput> GetWordbookItem(long id)
        {
            ...
            if (wordbook == null) throw new BusinessException($"系统中不存在Id为{wordbookItem.WordbookId}的字典类型");
        ...
        public async Task<IEnumerable<GetWordbookItemOutput>> GetWordbookItemsByCode(string code)
        {
            ...
            if (wordbook == null) throw new BusinessException($"系统中不存在code为{code}的字典类型");

            var wordbookItems = await _wordbookItemRepository.GetAllAsync(p => p.WordbookId == wordbook.Id);
            var wordbookItemOutputs = wordbookItems.MapTo<IEnumerable<GetWordbookItemOutput>>().Select(p =>
            {
        ...
        public async Task<GetWordbookItemOutput> GetWordbookItemByKey(string wordBookCode, string key)
        {
            ...
            var wordbookItem =
                await _wordbookItemRepository.SingleOrDefaultAsync(p => p.WordbookId == wordbook.Id && p.Key == key);
        ...
        public async Task<IPagedResult<GetWordbookItemOutput>> GetWordbookItems(GetWordbookItemsInput input)
        {
            ...
            else
            {
                ...
                if (wordbook == null) throw new BusinessException($"系统中不存在Code为{input.Code}的字典类型");
            }

            var wordbookItems = await _wordbookItemRepository.GetPageAsync(p => p.WordbookId == input.WordbookId,
                input.PageIndex, input.PageCount, new Dictionary<string, SortType> {{"Sort", SortType.Desc}});
        ...
        public async Task UpdateWordbook(UpdateWordbookInput input)
        {
            ...
            if (wordbook == null) throw new BusinessException($"系统中不存在Id为{input.Id}的字典类型");
            if (wordbook.IsSysPreset && wordbook.Code != input.Code) throw new BusinessException("不允许修改系统预设的字典类型标识");
            wordbook = input.MapTo(wordbook);
        ...
        public async Task UpdateWordbookItem(UpdateWordbookItemInput input)
        {
            ...
            if (wordbookItem == null) throw new BusinessException($"系统中不存在Id为{input.Id}的字典项");
            var wordbook = await _wordbookItemRepository.SingleOrDefaultAsync(p => p.Id == wordbookItem.WordbookId);
            if (wordbookItem == null) throw new BusinessException($"系统中不存在Id为{wordbookItem.WordbookId}的字典类型");
            wordbookItem = input.MapTo(wordbookItem);
